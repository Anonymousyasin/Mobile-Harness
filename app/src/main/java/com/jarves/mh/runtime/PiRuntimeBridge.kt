...
class PiRuntimeBridge(
    ...
    private fun buildContextPrompt(currentPrompt: String, history: List<ChatMessage>, guestWorkspacePath: String, projectKind: ProjectKind): String {
        ...
        if (projectKind == ProjectKind.QUICK_PROJECT) {
            sb.appendLine("This is a lightweight chat workspace at $guestWorkspacePath.")
            sb.appendLine("Be natural and concise. Only use terminal or file tools when the request needs them. Keep every file and command inside this workspace.")
            sb.appendLine("Do not describe the workspace setup unless asked.")
        } else {
